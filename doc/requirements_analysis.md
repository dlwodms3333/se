...
| Associated Use Case | UC01, UC02, UC03, UC04, UC05, UC06, UC07, UC08 |
| Responsibilities | 로그인, 파일 업로드, 파일 다운로드, 파일 검색, 파일 공유, 폴더 관리 |
| Collaborators | File, Folder, SharePermission, ExternalLink, FileVersion, ActivityLog |

| 구분 | 내용 |
|---|---|
| Attributes | userId, email, password, role |
| Operations | login(), uploadFile(), downloadFile(), searchFile(), shareFile() |
...
| Associated Use Case | UC02, UC03, UC05, UC06, UC07, UC08 |
...
| ID | 제약사항 |
|---|---|
| CR-001 | Mini Drive는 조직 내부 사용자를 주요 대상으로 한다. |
| CR-002 | 시스템은 웹 브라우저를 통해 접근할 수 있어야 한다. |
| CR-003 | 시스템은 별도의 프로그램 설치 없이 사용할 수 있도록 설계한다. |
| CR-004 | 시스템은 일반적인 업무 환경에서 사용하는 문서, 이미지, PDF, 압축 파일 등을 지원해야 한다. |
| CR-005 | 시스템은 약 200명 규모의 조직에서 사용할 수 있도록 설계한다. |
| CR-006 | 시스템은 권한 기반 접근 제어를 제공해야 한다. |
| CR-007 | 외부 공유 링크는 만료 기간과 비활성화 상태를 관리할 수 있어야 한다. |
| CR-008 | 주요 장애 발생 시 24시간 이내 복구를 목표로 한다. |
| CR-009 | 향후 공동 편집, 자동 분류, AI 기반 검색 기능을 추가할 수 있도록 확장성을 고려한다. |
...
| 요구사항 | UC01<br>로그인 | UC02<br>파일 업로드 | UC03<br>파일 다운로드 | UC04<br>폴더 관리 | UC05<br>파일 공유 | UC06<br>외부 링크 생성 | UC07<br>파일 검색 | UC08<br>버전 관리 | UC09<br>계정 관리 | UC10<br>로그 확인 | UC11<br>링크 접근 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| FR-001 |  | O |  |  |  |  |  |  |  |  |  |
| FR-002 |  |  | O |  |  |  |  |  |  |  |  |
| FR-003 |  | O |  |  |  |  |  |  |  |  |  |
